...
public:
    ...
    void initializeTransferModel(Mutation::Mixture& mix)
    {
        ...
        try {
            ...
            addTransferTerm(0, 1, Factory::create("OmegaVT", mix));
            addTransferTerm(0, 1, Factory::create("OmegaCV", mix));
            addTransferTerm(0, 1, Factory::create("OmegaCElec", mix));

            // Terms only included when electrons are present
            if (m_thermo.hasElectrons()) {
                addTransferTerm(0, 1, Factory::create("OmegaET", mix));
                addTransferTerm(0, 1, Factory::create("OmegaCE", mix));
                addTransferTerm(0, 1, Factory::create("OmegaI", mix));
            }
        } catch (Error& e) {
